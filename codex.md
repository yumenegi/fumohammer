# FUMOHAMMER CODEX
### Character Roster — Draft v0.1

> Companion document to [rules.md](rules.md) — see Section 9 (Character Cards) and Section 10 (Roster) there for how these cards plug into the systems. This file holds the actual stat blocks; the rulebook only defines what the fields mean.

> **Note on numbers:** rules.md hasn't finalized base damage/HP magnitudes yet (see its Section 12.5 changelog and the open balance questions from early drafting). The values below use the stat cap already locked in (1–10, maybe 12 — rules.md Section 4.1) and a damage range consistent with that cap, but treat every number here as provisional until a playtest confirms the base math.

---

## Card Template

Copy this block for any new character.

**[Name]**
- **Native Faction:** Gensokyolites / Lunarians / Flexible — flavor + triggers the match bonus in rules.md Section 9.2. Does not restrict which team the fumo can actually be assigned to.
- **Zone Affinity:** —
- **Lore blurb:** —

| Stat | Value |
|---|---|
| HP | |
| Spirit (max cap) | |
| Speed | |
| Defense | |

**Danmaku Moves**
| Name | Tier | Spirit Cost | Range | Damage | Notes |
|---|---|---|---|---|---|
| | | | | | |

**Melee Moves**
| Name | Tier | Spirit Cost | Damage | Notes |
|---|---|---|---|---|
| | | | | |

**Spell Cards** (uses per game — rules.md Section 4.7)
| Name | Spirit Cost | Uses/Game | Tier | Effect |
|---|---|---|---|---|
| | | | | |

---

## Roster

### Cirno
- **Native Faction:** Gensokyolites
- **Zone Affinity:** None
- **Lore blurb:** An ice fairy of the Misty Lake, convinced she's the strongest thing in Gensokyo. Mostly right about the "strongest fairy" part.

| Stat | Value |
|---|---|
| HP | 7 |
| Spirit (max cap) | 6 |
| Speed | 8 |
| Defense | 3 |

**Danmaku Moves**
| Name | Tier | Spirit Cost | Range | Damage | Notes |
|---|---|---|---|---|---|
| Icicle Fall | 1 | 1 | Short cone | 2 | Freely grazeable poke |
| Ice Sign: Icicle Machine Gun | 2 | 3 | Medium line | 4 | Rapid volley |

**Melee Moves**
| Name | Tier | Spirit Cost | Damage | Notes |
|---|---|---|---|---|
| Frozen Fist | 1 | 1 | 3 | Close-range ice punch |

**Spell Cards**
| Name | Spirit Cost | Uses/Game | Tier | Effect |
|---|---|---|---|---|
| Perfect Freeze | 6 | 1 | 3 | 6 damage to one target; TODO — intended to also apply a "frozen" status (−2 Speed for 1 round), but rules.md's Status Effects table (Section 7.6) is still empty, so this half of the effect has no ruleset to plug into yet |

---

### Reimu Hakurei
- **Native Faction:** Gensokyolites
- **Zone Affinity:** None
- **Lore blurb:** The shrine maiden of the Hakurei Shrine, sitting on the border of Gensokyo. Deals with incidents because someone has to, and it's usually faster than waiting for an explanation.

| Stat | Value |
|---|---|
| HP | 9 |
| Spirit (max cap) | 8 |
| Speed | 7 |
| Defense | 6 |

**Danmaku Moves**
| Name | Tier | Spirit Cost | Range | Damage | Notes |
|---|---|---|---|---|---|
| Homing Amulet | 1 | 1 | Medium, homing | 2 | Curves toward target within cone |
| Persuasion Needle | 2 | 3 | Medium line | 4 | Rapid needle volley |

**Melee Moves**
| Name | Tier | Spirit Cost | Damage | Notes |
|---|---|---|---|---|
| Gohei Strike | 1 | 1 | 3 | Exorcism wand strike |

**Spell Cards**
| Name | Spirit Cost | Uses/Game | Tier | Effect |
|---|---|---|---|---|
| Fantasy Seal | 5 | 2 | 3 | 5 damage; template hits every enemy fumo caught in it, not just one target (existing template-based multi-hit already supported per rules.md Section 6.2) |

---

### Marisa Kirisame
- **Native Faction:** Gensokyolites
- **Zone Affinity:** None
- **Lore blurb:** A human magician who'll borrow anything not nailed down, including magic. Prefers to solve problems by pointing a very large laser at them.

| Stat | Value |
|---|---|
| HP | 7 |
| Spirit (max cap) | 9 |
| Speed | 8 |
| Defense | 4 |

**Danmaku Moves**
| Name | Tier | Spirit Cost | Range | Damage | Notes |
|---|---|---|---|---|---|
| Magic Missile | 1 | 1 | Medium | 2 | Basic bolt |
| Illusion Laser | 2 | 3 | Long line | 4 | Thin piercing beam |

**Melee Moves**
| Name | Tier | Spirit Cost | Damage | Notes |
|---|---|---|---|---|
| Broom Whack | 1 | 1 | 2 | Weakest melee in the roster — Marisa isn't built for close range |

**Spell Cards**
| Name | Spirit Cost | Uses/Game | Tier | Effect |
|---|---|---|---|---|
| Master Spark | 7 | 1 | 3 | 8 damage (highest single hit in the roster); long line template, hits every fumo caught in it. Costs nearly all of Marisa's Spirit cap in one shot |

---

### Tewi Inaba
*(template — not yet filled in)*
- **Native Faction:**
- **Zone Affinity:**
- **Lore blurb:**

| Stat | Value |
|---|---|
| HP | |
| Spirit (max cap) | |
| Speed | |
| Defense | |

**Danmaku Moves**
| Name | Tier | Spirit Cost | Range | Damage | Notes |
|---|---|---|---|---|---|
| | | | | | |

**Melee Moves**
| Name | Tier | Spirit Cost | Damage | Notes |
|---|---|---|---|---|
| | | | | |

**Spell Cards**
| Name | Spirit Cost | Uses/Game | Tier | Effect |
|---|---|---|---|---|
| | | | | |

---

### Aya Shameimaru
*(template — not yet filled in)*
- **Native Faction:**
- **Zone Affinity:**
- **Lore blurb:**

| Stat | Value |
|---|---|
| HP | |
| Spirit (max cap) | |
| Speed | |
| Defense | |

**Danmaku Moves**
| Name | Tier | Spirit Cost | Range | Damage | Notes |
|---|---|---|---|---|---|
| | | | | | |

**Melee Moves**
| Name | Tier | Spirit Cost | Damage | Notes |
|---|---|---|---|---|
| | | | | |

**Spell Cards**
| Name | Spirit Cost | Uses/Game | Tier | Effect |
|---|---|---|---|---|
| | | | | |

---

### Sanae Kochiya
*(template — not yet filled in)*
- **Native Faction:**
- **Zone Affinity:**
- **Lore blurb:**

| Stat | Value |
|---|---|
| HP | |
| Spirit (max cap) | |
| Speed | |
| Defense | |

**Danmaku Moves**
| Name | Tier | Spirit Cost | Range | Damage | Notes |
|---|---|---|---|---|---|
| | | | | | |

**Melee Moves**
| Name | Tier | Spirit Cost | Damage | Notes |
|---|---|---|---|---|
| | | | | |

**Spell Cards**
| Name | Spirit Cost | Uses/Game | Tier | Effect |
|---|---|---|---|---|
| | | | | |

---

### Yorigami Shion
*(template — not yet filled in)*
- **Native Faction:**
- **Zone Affinity:**
- **Lore blurb:**

| Stat | Value |
|---|---|
| HP | |
| Spirit (max cap) | |
| Speed | |
| Defense | |

**Danmaku Moves**
| Name | Tier | Spirit Cost | Range | Damage | Notes |
|---|---|---|---|---|---|
| | | | | | |

**Melee Moves**
| Name | Tier | Spirit Cost | Damage | Notes |
|---|---|---|---|---|
| | | | | |

**Spell Cards**
| Name | Spirit Cost | Uses/Game | Tier | Effect |
|---|---|---|---|---|
| | | | | |

---

### Yuyuko Saigyouji
*(template — not yet filled in)*
- **Native Faction:**
- **Zone Affinity:**
- **Lore blurb:**

| Stat | Value |
|---|---|
| HP | |
| Spirit (max cap) | |
| Speed | |
| Defense | |

**Danmaku Moves**
| Name | Tier | Spirit Cost | Range | Damage | Notes |
|---|---|---|---|---|---|
| | | | | | |

**Melee Moves**
| Name | Tier | Spirit Cost | Damage | Notes |
|---|---|---|---|---|
| | | | | |

**Spell Cards**
| Name | Spirit Cost | Uses/Game | Tier | Effect |
|---|---|---|---|---|
| | | | | |

---

## Notes

- All eight characters here are canonically Gensokyo residents — none are Lunarians. Since faction assignment is now free (rules.md Section 9.2), any of them can still be fielded on Side B; they just won't get the Native Faction match bonus there. Worth deciding whether that's fine as-is, or whether you want at least one lunar-affiliated card (e.g. an Eirin/Kaguya/Reisen-type kit) in the pool so Side B has a shot at the bonus too.
- The Native Faction match bonus itself (Section 9.2) is still a TODO in rules.md — no effect or magnitude defined yet.
- Spell Card damage and Spirit costs above assume the stat caps already locked into rules.md (Section 4.1); they have not been playtested.
